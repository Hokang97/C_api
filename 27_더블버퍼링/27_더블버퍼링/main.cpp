...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_CREATE:
		bitmap.LoadBitmapByPath("Resource/sample.bmp");
		bitmap.SetTransparentColor(RGB(255, 255, 255));
	...
	case WM_TIMER:
		...
		// 백버퍼에 새로운 도화지를 설정한 후 그리기
		SelectObject(BackmemDC, hBit);
		FillRect(BackmemDC, &rect, (HBRUSH)GetStockObject(DKGRAY_BRUSH));
		bitmap.DrawBitmapByCropping(BackmemDC, 0, 0, 500, 500, (index % 2) * 100, (index / 2) * 100, 100, 100);
